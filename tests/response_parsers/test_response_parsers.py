...
def test_get_yaml_from_response_valid_yaml():
    ...
    tagged_content = get_yaml_from_response(response.response)
...
def test_get_json_from_response_valid_json():
    ...
    tagged_content = get_json_from_response(response.response)
...
def test__get_format_from_response():
    response = AIResponse(_response="```json\n{\"key\": \"value\"}\n```")
    tags = [Tag("```json", "```", 0)]
    content, start_ind, end_ind = _get_format_from_response(response.response, tags)
...
def test__get_format_from_response_no_tags():
    ...
    tags = [Tag("```json", "```", 0)]
    content, start_ind, end_ind = _get_format_from_response(response.content, tags)
